...
%% Load
load(MGBPATHs{1}, "CTLParams");
pos = CTLParams.MGBI2(:); % ms
window = CTLParams.Window; % ms
dur = CTLParams.S1Duration(1); % ms

dataMGB = cellfun(@(x) load(x).chSpkRes', MGBPATHs, "UniformOutput", false);

for index = 1:length(dataMGB)
    dataMGB{index} = addfield(dataMGB{index}, "pos", num2cell(pos));

    temp = {dataMGB{index}.chSPK}';
    ...
    temp = cellfun(@(x) addfield(x, "cluster", cellfun(@(y) strcat(recordID_MGB{index}, '-', y), {x.cluster}', "UniformOutput", false)), temp, "UniformOutput", false);

    temp = cat(1, temp{:});

    dataMGB{index} = temp;
end
dataMGB = cat(1, dataMGB{:});

%% 
clustersMGB = unique({dataMGB.cluster}', 'stable');
dfr = cell(numel(pos) - 1, numel(clustersMGB));
[latencyOnset, latencyOffset] = deal(nan(numel(clustersMGB), 1));
[p, tval] = deal(cell(size(dfr, 2), 1));
X = pos(2:end);

for cIndex = 1:numel(clustersMGB)
    close all;

    temp = dataMGB(strcmp({dataMGB.cluster}, clustersMGB{cIndex}));
    ...
    addTitle2Fig(Fig, clustersMGB{cIndex});
    exportgraphics(Fig, ['..\Figures\NP\MGB\', clustersMGB{cIndex}, '.jpg'], "Resolution", 300);
end
